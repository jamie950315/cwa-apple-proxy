...
def render_rules(config,status,prefixes,replace=False):
    ...
    # Reject before route lookup so missing public IPv6 egress cannot stall QUIC.
...
def apply():
    ...
    result={'transportVersion':'1.0.2','time':int(time.time()),'mode':'split-dns-relay-routes','clients4':c4,'clients6':c6,
            'relayPrefixes':prefixes,'relayRouteApproval':approval,'requiredClientExitNode':None,'exitNodeSelection':'optional',
            'policy':'Enrolled clients: UDP/443 to Pi5 rejected on input; UDP/443 to selected Apple relay subnets rejected before/at forward. IPv6 relay TCP/443 is reset only when the FIB has no destination route; IPv4 TCP and other destinations pass unchanged.',
