...
const list=JSON.parse(fs.readFileSync(path.join(base,'research/approved-split/native-proof-list.json')));
const items=[];
for(const folder of list){
 try{
  ...
  const aq=WeatherKit2.decode(new ByteBuffer(output),['airQuality']).airQuality;
  if(snap.airQuality)assert.equal(aq.index,snap.airQuality.index);
  items.push({status:'passed',time:req.time,app:req.app,folder:path.relative(base,folder),checkedFields:checked,skipped:report.skippedFields,temperature:root.currentWeather().temperature(),sourceTemperature:snap.current.temperature,station:req.stationName,coverage:report.coverage,inputBytes:original.length,outputBytes:output.length});
 }catch(e){items.push({status:'failed',folder,error:String(e)})}
}
const report={count:items.length,passed:items.filter(x=>x.status==='passed').length,items};
fs.writeFileSync(path.join(base,'research/approved-split/native-audit.json'),JSON.stringify(report,null,2));
console.log(JSON.stringify({count:report.count,passed:report.passed}));
